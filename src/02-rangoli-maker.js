...
export function extractRangoliCenter(design, start, end) {
  ...
  let data = design.slice(start, end);
  return data;
}
...
export function splitAndJoinRangoli(colorString, oldSep, newSep) {
  ...
  let data = colorString.split(oldSep);
  let joindata = data.join(newSep);
  return joindata;
}
...
export function makeRangoliBorder(char, length) {
  ...
  let newBorder = border.slice(0, length);
  return newBorder;
}
